"""
build_phase2_kitchen_swap.py — Phase 2 branch point: Kitchen/Living Room swap.

Builds the connected baseline (identical to build_connected_baseline.py) then adds the
monolithic kitchen FF&E to the front-right (old great-room/bay) footprint:
  - Sub-Zero 36" right-opening refrigerator, flush-integrated floor-to-ceiling walnut panels
  - Continuous back counter run + Wolf cooktop centered, concealed/enclosed hood in upper casework
  - 36" floor-to-ceiling pantry abutting the 2.5' box bay (right side)
  - 12' x 4' Calacatta Viola island, 48" clearance to back counter
  - Cove dishwasher + under-mount sink on working side of island
  - Dining zone (y130-272) kept entirely clear
  - Living room in front-left (old kitchen footprint) — shell only, no FF&E
  - Materials: Walnut (70,48,34), Calacatta Viola (230,219,225)

Rollback: run build_connected_baseline.py to return to the bare coordinate grid.

USAGE:
    pip install "mcp>=1.0.0" httpx
    export TRIMBLE_ACCESS_TOKEN=...
    python build_phase2_kitchen_swap.py    # -> colby_st_PHASE2_kitchen_swap.skp
"""
...
OUT_FILE = "colby_st_PHASE2_kitchen_swap.skp"
...
box(K,"BackCounter_base",160,10,107,90,24,36,matwal)
box(K,"BackCounter_top",158,8,143,94,28,2.5,matstone)
box(K,"Wolf_Cooktop",190,12,143,30,20,2,matcook)
box(K,"Concealed_Hood_Casework",160,10,160,90,22,43,matwal)
box(K,"Pantry_FloorToCeiling",254,8,107,36,36,96,matwal)
box(K,"Island_base",133,82,107,144,48,36,matwal)
box(K,"Island_top",130,79,143,150,54,2.5,matstone)
box(K,"Sink_undermount",190,90,141,30,18,2.5,matsink)
box(K,"Cove_Dishwasher",150,82,107,24,24,34,matwal)
K.set_layer(lay_up)
