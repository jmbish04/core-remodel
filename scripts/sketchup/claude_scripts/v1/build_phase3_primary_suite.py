"""
build_phase3_primary_suite.py — Phase 3 branch point: Primary Suite massing.

Builds from scratch: connected baseline (cantilevers, stacked plumbing) + Phase 2 monolithic
kitchen + Phase 3 primary suite FF&E in Moody Modern schematic massing.

Phase 3 adds three named groups to Upper_Level:
  PrimaryBath_Fixtures
    - Curbless walk-in shower (52"x38", dark slate) on the stair wall / plumbing stack
    - Floating double vanity (walnut, 18" AFF) on back wall, Calacatta Viola top + double basins
    - Full-height mirror panel behind vanity
    - Freestanding oval soaking tub in wet zone
    - Wall-hung toilet w/ concealed in-wall tank, right side
  PrimaryCloset_Builtin
    - Left wall, right wall, and back wall runs of 24"-deep floor-to-ceiling walnut panels
    - Closet zone y491-563 (72" deep off bedroom door)
  PrimaryBed_Furniture
    - 72"x84" low-profile platform bed (walnut 8" platform + mattress), sleeping zone y563-723
    - 40" headboard panel against back wall
    - Two floating nightstands (29" AFF)
  Sightline axis x200-236: bedroom door → closet opening → bath, linear and unobstructed

Rollback chain: build_connected_baseline.py → build_phase2_kitchen_swap.py → this file.

USAGE:
    pip install "mcp>=1.0.0" httpx
    export TRIMBLE_ACCESS_TOKEN=...
    python build_phase3_primary_suite.py    # -> colby_st_PHASE3_primary_suite.skp
"""
...
OUT_FILE = "colby_st_PHASE3_primary_suite.skp"
...
matwal=getmat("Walnut",70,48,34); matstone=getmat("Calacatta_Viola",230,219,225)
matcook=getmat("Cooktop",40,40,46); matsink=getmat("Sink",70,72,78)
matslate=getmat("Dark_Stone",52,48,50); matceramic=getmat("Ceramic_White",240,238,234)
matmetal=getmat("Brushed_Metal",168,172,176)
lay_lo=Layer(); lay_lo.set_name("Lower_Level"); lay_up=Layer(); lay_up.set_name("Upper_Level")
model.add_layers([lay_lo,lay_up])
LOWER=__LOWER__; UPPER=__UPPER__
...
K=Group(); model.get_entities().add_group(K); K.set_name("Kitchen_Monolithic")
box(K,"Fridge_SubZero36",120,6,107,36,30,96,matwal)
box(K,"BackCounter_base",160,10,107,90,24,36,matwal); box(K,"BackCounter_top",158,8,143,94,28,2.5,matstone)
box(K,"Wolf_Cooktop",190,12,143,30,20,2,matcook); box(K,"Concealed_Hood_Casework",160,10,160,90,22,43,matwal)
box(K,"Pantry_FloorToCeiling",254,8,107,36,36,96,matwal)
box(K,"Island_base",133,82,107,144,48,36,matwal); box(K,"Island_top",130,79,143,150,54,2.5,matstone)
box(K,"Sink_undermount",190,90,141,30,18,2.5,matsink); box(K,"Cove_Dishwasher",150,82,107,24,24,34,matwal)
K.set_layer(lay_up)

Z=107.0
PB=Group(); model.get_entities().add_group(PB); PB.set_name("PrimaryBath_Fixtures")
box(PB,"Shower_Enclosure",228,390,Z,52,38,96,matslate); box(PB,"Shower_Niche",228,395,Z+60,2,20,14,matmetal)
box(PB,"Vanity_base",227,463,Z+18,63,21,20,matwal); box(PB,"Vanity_top",226,462,Z+38,65,23,1.5,matstone)
box(PB,"Sink_L",231,465,Z+38,20,14,2,matceramic); box(PB,"Sink_R",253,465,Z+38,20,14,2,matceramic)
box(PB,"Mirror_Panel",227,483,Z+38,63,2,58,matmetal)

box(PB,"Toilet_WallHung",275,420,Z+15,15,28,18,matceramic); box(PB,"Toilet_InWall_Tank",281,418,Z+33,6,8,24,matwal)
PB.set_layer(lay_up)

PC=Group(); model.get_entities().add_group(PC); PC.set_name("PrimaryCloset_Builtin")
box(PC,"Closet_Left",154,491,Z,24,72,96,matwal); box(PC,"Closet_Right",268,491,Z,24,72,96,matwal)
box(PC,"Closet_Back",178,539,Z,90,24,96,matwal)
PC.set_layer(lay_up)

BED=Group(); model.get_entities().add_group(BED); BED.set_name("PrimaryBed_Furniture")
box(BED,"Bed_Platform",186,610,Z,72,84,8,matwal); box(BED,"Bed_Mattress",188,612,Z+8,68,80,10,matceramic)
box(BED,"Headboard",184,690,Z,76,3,40,matwal)
box(BED,"Nightstand_L",154,630,Z+22,18,16,3,matwal); box(BED,"Nightstand_R",269,630,Z+22,18,16,3,matwal)
BED.set_layer(lay_up)
