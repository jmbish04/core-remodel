...
- **Flat contract stays flat.** Do NOT change the JSON keys any endpoint or MCP tool returns,
  ...
  Frontend + all 13 response TS types must not need edits (except removing the two dead
  `distance_from_sf_*` field defs in Phase C).
...
2. `A2` Add `src/backend/db/schema/showroom/store_location.ts` → table `showroom_store_locations` (columns per the plan's target ERD; `store_id` UNIQUE FK cascade; `place_id` unique index nulls-distinct; `bay_area_city_id` FK set-null; **notes as a PlateJS triple** `notes` (plaintext) + `notes_markdown` + `notes_html`; timestamps). Export from `showroom/index.ts`. `pnpm run db:generate`.
3. `A3` `pnpm run migrate:remote`; verify the empty table exists remotely.
4. `A4` Write `scripts/backfill/showroom-locations.mjs`: 1:1 copy (COALESCE zip), seed the notes triple from the existing plain `location_notes` (`notes`=plain, `notes_markdown`=plain, `notes_html`=`<p>`-escaped), gap-fill street parts from `location_address` via a small `parseFormattedUsAddress` (ship an `assert`-based self-check), and migrate `main_poc_*` → contacts (named → `SALES` first/last split + `mobile_phone_number` + email; name-less → upsert `GENERAL_CONTACT`). Idempotent (upsert by `store_id`; guard contact inserts).
...
1. `B1` Shared read helper `loadStoreWithLocation` — LEFT JOIN `showroom_store_locations`, merge to flat camelCase, derive `hubRoute`/`hubName` from the `bay_area_city` join, falling back to `resolveCityName(signals)`.
...
6. `B9` Notes → PlateJS triple end-to-end: the field-out writer accepts `{markdown, html}` (derives plaintext) and the read helper returns `locationNotes`/`locationNotesMarkdown`/`locationNotesHtml`; swap the notes input in `EditStoreModal.tsx` (and the intake notes field) from a plain textarea to the existing `OverviewNoteEditor` (`@/components/showroom/OverviewNoteEditor`). Sanitize html on write.
7. `B8` `scripts/qc/pr_<B>.mjs` on `--preview` AND prod; frontend smoke on preview (notes editor round-trips markdown+html). Changelog + link.
