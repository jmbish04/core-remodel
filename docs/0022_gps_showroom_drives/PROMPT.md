...
- **DB integrity + timeouts (from review):** push invariants to the database, not just app code — `showroom_visit_log` gets a `rating BETWEEN 1 AND 5` CHECK, a `((store_id IS NOT NULL) <> (hitl_queue_id IS NOT NULL))` XOR CHECK, and a partial UNIQUE index on `soft_arrival_id`. Every external fetch in background/webhook paths (`proximityScan` → Places/Gemini, `navigate-drive` → Tessie) sets an `AbortSignal.timeout` (10s scans, 15s Tessie), matching the existing `tesla.ts` pattern. Drizzle CHECK constraints: use `check()` in the table builder; verify the generated migration includes them.
