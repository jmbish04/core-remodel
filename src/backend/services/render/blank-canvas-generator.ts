...
export async function generateBlankCanvas(
  ...
  try {
    AI_GATEWAY_TOKEN = await getCloudflareAiGatewayToken(env);
    console.log(
      `[generateBlankCanvas] Resolved CLOUDFLARE_AI_GATEWAY_TOKEN (exists: ${!!AI_GATEWAY_TOKEN}, len: ${AI_GATEWAY_TOKEN?.length || 0})`,
    );
  } catch (gateErr: any) {
    console.error(
      `[generateBlankCanvas] Failed to retrieve CLOUDFLARE_AI_GATEWAY_TOKEN:`,
      gateErr,
    );
    throw new Error(
      `Failed to retrieve CLOUDFLARE_AI_GATEWAY_TOKEN: ${gateErr.message || gateErr}`,
    );
  }
