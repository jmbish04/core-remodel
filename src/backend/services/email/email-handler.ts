 ...
 * (against the store's WEBSITE link domain) or a fuzzy name match. Returns the
 * store id, or null. Public providers (gmail etc.) never domain-match.
 */
 ...
 * showroom when one matches the domain/name; otherwise saves a DRAFT contact
 * (is_draft) so the HITL inbox can map it to a showroom (or spin up a new one).
 * Deduplicates on the sender email so repeat mail doesn't pile up contacts.
 *
 * Only runs when the email did NOT match a directory company — a matched
 * company is already handled by the company CRM.
 */
async function registerShowroomContactFromEmail(
  senderEmail: string | null,
  senderName: string | null,
...
async function processEmail(
  ...
  // Only when the sender did NOT match a directory company: register a
  // showroom contact from the signature — mapped to a showroom by domain/name,
  // else saved as a draft for the HITL inbox to map. Never breaks classification.
  if (!companyMatch.companyId) {
    try {
      ...
      console.error("[email-handler] showroom contact auto-populate failed:", err);
    }
  }
