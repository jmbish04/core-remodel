...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    changes: [
      { kind: "migration", text: "0167 (additive): worker_email_invoice_line_items.product_id (FK→products) + brand_id (FK→brands) + index. Applied to remote + verified." },
      { kind: "added", text: "services/email/map-invoice-products.ts — mapInvoiceLinesToProducts(): reuses ensureProductFromExtraction (brand+item dedup), links via showroom_product_mappings (idempotent uniq), records product_price_observations (deduped on product+store+cents), stamps line product_id/brand_id/match_status." },
      { kind: "changed", text: "email pipeline fires product mapping after line-item insert + material deduction; best-effort (a failure never breaks email processing)." },
      { kind: "changed", text: "GET /:id/pending-quotes lines now carry productId/brandId/productName (JOINed from products, never denormalized)." },
      { kind: "added", text: "PendingQuotesPanel: each mapped line shows the product + a 'new from quote' / 'matched' badge." },
    ],
    migrations: ["0167"],
    status: "staged",
  },
  ...
  },
  {
    ...
    changes: [
      { kind: "migration", text: "0158_famous_warpath: gmail_messages gains classification (enum), is_spam, spam_rationale, deleted_at; new gmail_message_images table. 0159_skinny_trish_tilby: gmail_messages.images_extracted guard. Additive; applied + verified on remote D1." },
      { kind: "added", text: "Embedded (cid:) images: on first view of a message, inline image parts are fetched from Gmail and uploaded to Cloudflare Images (ImageProcessorService), cached in gmail_message_images and shown in the reading pane; images_extracted guards it to one fetch+upload per message. Remote-hosted marketing images (external <img src>) are untouched." },
