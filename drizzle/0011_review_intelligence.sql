...
CREATE UNIQUE INDEX IF NOT EXISTS `image_tags_slug_unique` ON `image_tags` (`slug`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS `image_tag_mappings_image_tag_unique` ON `image_tag_mappings` (`image_id`,`tag_id`);--> statement-breakpoint

CREATE TABLE IF NOT EXISTS `image_review_highlights` (
  `id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
  `image_id` text NOT NULL,
