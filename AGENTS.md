...
* **Linting:** `pnpm run lint` (runs `oxlint`)
* **Formatting:** `pnpm run fmt` (runs `oxfmt`) - **WARNING:** Running `pnpm run fmt` globally can cause massive unintended formatting changes across thousands of files. Target only specific modified files!
...
* **Verify Branch:** As the first action of every session, verify the branch is fresh by running `pnpm run worktree:check` (or `node scripts/worktree-check.mjs`) before reading source files, dispatching explore agents, or answering analytical questions.
